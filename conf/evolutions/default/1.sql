...
alter table permission add constraint fk_permission_user_2 foreign key (user_id) references users (id);
create index ix_permission_user_2 on permission (user_id);
alter table result add constraint fk_result_user_3 foreign key (user_id) references users (id);
